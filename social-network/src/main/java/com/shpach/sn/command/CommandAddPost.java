 ...
 *
 */
public class CommandAddPost implements ICommand {
	private static final Logger logger = Logger.getLogger(CommandAddPost.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse responce)
		...
		if (postText != null) {
			...
			if (PostService.getInstance().addNewPost(post)) {
				request.setAttribute("addPostSuccessAlert", true);

			} else {
				request.setAttribute("addPostFailAlert", true);
			}
		} else {
			request.setAttribute("addPostFailAlert", true);
		}
