 ...
 *
 */
public class CommandDeletePost implements ICommand {
	private static final Logger logger = Logger.getLogger(CommandDeletePost.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse responce)
		...
		if (postId>0 && PostService.getInstance().deletePost(postId)){
			request.setAttribute("deletePostSuccessAlert", true);

		} else {
			request.setAttribute("deletePostFailAlert", true);
		}
