 ...
 *
 */
public class CommandApproveFriend implements ICommand {
	private static final Logger logger = Logger.getLogger(CommandApproveFriend.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse responce)
		...
		if (user!=null && userFriend!=null){
			...
			friend.setHostUser(userFriend);
			friend.setSlaveUser(user);
			friend.setFriendStatus(FriendService.friendStatus.FRIEND.ordinal());
			if(FriendService.getInstance().update(friend)){
				request.setAttribute("approveFriendSuccessAlert", true);
			}else{
				request.setAttribute("approveFriendFailAlert", true);
			}
		}
